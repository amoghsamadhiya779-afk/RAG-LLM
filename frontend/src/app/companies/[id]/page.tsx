...
export default function CompanyPage() {
  ...
  if (isLoading) {
    return <div className="container-page py-16 animate-pulse h-[400px] bg-muted/20 rounded-xl" />;
  }

  if (!data || !data.company) {
    return (
      <div className="container-page py-20 text-center text-muted-foreground">
        Company not found. <Link href="/companies" className="underline hover:text-primary">Browse companies</Link>.
      </div>
    );
  }

  const { company, jobs } = data;
  const withCompany: JobWithCompany[] = jobs.map((j: any) => ({ ...j, company }));
        ...
        <section className="container-page py-12">
          ...
          {withCompany.length === 0 ? (
            <div className="mt-6 glass-card p-10 text-center text-muted-foreground">
              No open roles right now. <Link href="/jobs" className="underline hover:text-primary">Browse other companies</Link>.
            </div>
          ) : (
