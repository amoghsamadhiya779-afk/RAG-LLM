...
export function ApplyDialog({ job, size = "default" }: ApplyDialogProps) {
  ...
  const mutation = useMutation({
    ...
    onMutate: async () => {
      ...
      setOpen(false);
      void fireConfetti();
      toast.success(`Application sent to ${typeof job.company === "string" ? job.company : job.company?.name || "Unknown"}`);
    },
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Submitting to <span className="text-foreground">{typeof job.company === "string" ? job.company : job.company?.name || "Unknown"}</span>. Your default resume will be attached.
          </DialogDescription>
        </DialogHeader>
