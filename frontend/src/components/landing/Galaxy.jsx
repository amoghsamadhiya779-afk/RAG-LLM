...
// Star-field shader adapted from the React Bits "Galaxy" component.
// Draws a rotating layered star field with twinkle, glow, hue shift and
// optional mouse repulsion. Pure GLSL — no textures.
const fragment = /* glsl */ `
...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float glossLocal = tris(uStarSpeed / (PERIOD * seed + 1.0));
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSpeed: { value: speed },
        uMouse: { value: [smoothMousePos.current.x, smoothMousePos.current.y] },
    ...
    function update(t) {
      ...
      program.uniforms.uMouse.value = [smoothMousePos.current.x, smoothMousePos.current.y];
    ...
    focal[0], focal[1], rotation[0], rotation[1], starSpeed, density, hueShift,
    disableAnimation, speed, mouseInteraction, glowIntensity, saturation,
    mouseRepulsion, twinkleIntensity, rotationSpeed, repulsionStrength,
