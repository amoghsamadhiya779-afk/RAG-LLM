  ...
  return (
    <div className="overflow-hidden rounded-xl bg-muted">
      ...
      <div className="flex items-center gap-1.5 border-b border-border bg-card/40 px-3 py-2.5">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-border" />
        <span className="h-2.5 w-2.5 rounded-full bg-border" />
        <span className="ml-3 font-mono text-[11px] text-muted-foreground">jOBiON.dev/search</span>
      </div>
          ...
          {(items.length ? items : Array.from({ length: 3 })).map((j, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-gradient-accent px-2.5 py-1 text-[11px] font-semibold text-white">
                  {matches[i]}% match
                </span>
              </motion.li>
            );
