 ...
 <section id="ai-workspace" className={`relative py-24 sm:py-32 border-t border-bone/[0.06] ${className || ""}`}>
  ...
  <div className="grid gap-8 lg:grid-cols-2">
   ...
   <div className="flex items-center gap-3 mb-6">
    ...
    <div>
    <h2 className="text-body-lg font-heading">Semantic Search</h2>
    <p className="text-small text-secondary">Find roles using natural language.</p>
    </div>
   </div>
   
   <div className="flex gap-3 w-full">
    <div className="flex flex-1 items-center gap-3 rounded-pill border border-bone/10 bg-[#1d1d1d] px-4 py-1.5 ">
   ...
   <div className="mt-4 flex-1 rounded-inputs border border-dashed border-bone/10 bg-char/30 p-4 overflow-y-auto max-h-[300px] min-h-[150px]">
    ...
    <div className="h-full flex items-center justify-center text-small text-secondary animate-pulse py-8">
     Searching vector database for: "{searchQuery}"...
    </div>
    ) : (
    <div className="h-full flex items-center justify-center text-small text-secondary text-center py-8">
   ...
   <div className="flex items-center gap-3 mb-6">
    ...
    <div>
    <h2 className="text-body-lg font-heading">Instant Resume Parsing</h2>
    <p className="text-small text-secondary">Extract skills and experience instantly.</p>
    </div>
   </div>
   ...
   <div className="flex flex-col items-center justify-center gap-4 rounded-cards border-2 border-dashed border-bone/10 bg-char/30 p-10 text-center transition-colors hover:bg-char/50">
    ...
    <div>
    <p className="text-small font-ui">Click to upload or drag and drop</p>
    <p className="text-micro text-secondary">PDF (max. 5MB)</p>
    </div>
   ...
   {parsedSkills.length > 0 && (
    <div className="mt-4">
    <h3 className="text-small font-ui mb-2">Extracted Skills:</h3>
    <div className="flex flex-wrap gap-1.5">
     {parsedSkills.map((s, i) => (
     <span key={i} className="bg-primary/10 text-bone text-micro px-2.5 py-1 rounded-tags border border-bone/5">{s}</span>
     ))}
    </div>
    </div>
   )}
   ...
   <div className="flex items-center gap-3 mb-6">
    ...
    <div>
    <h2 className="text-body-lg font-heading">AI Matching & One-Click Apply</h2>
    <p className="text-small text-secondary">See how your parsed profile matches with live jobs.</p>
    </div>
   </div>
   ...
   <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
    ...
    matchedJobs.map((job) => (
     <div key={job.id} className="flex flex-col rounded-cards border border-bone/10 bg-char/40 p-5 transition-colors hover:bg-iron/40 hover:border-bone/20">
     <div className="flex items-start justify-between mb-2">
      <div className="h-10 w-10 rounded-md bg-void border border-bone/10 flex items-center justify-center font-display text-bone shrink-0">
      {job.company?.name?.[0]?.toUpperCase()}
      </div>
      <span className="inline-flex items-center rounded-pill bg-indigo-haze/20 px-2.5 py-0.5 text-micro font-ui text-bone border border-indigo-haze/30">
      {Math.floor(Math.random() * (99 - 85 + 1) + 85)}% Match
      </span>
     </div>
     <h3 className="font-heading text-body-lg line-clamp-1">{job.title}</h3>
    ...
    [1, 2, 3].map((i) => (
     ...
     <div className="flex items-start justify-between mb-2">
      <div className="h-10 w-10 rounded-md bg-void border border-bone/10" />
      <span className="inline-flex items-center rounded-pill bg-char border border-bone/10 px-2.5 py-0.5 text-micro font-ui text-fog">
      --% Match
      </span>
     </div>
   ...
   <div className="flex items-center gap-4 px-6 py-4 border-b border-bone/5 bg-char/20 z-10">
    ...
    <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full border border-void bg-emerald-400 -[0_0_8px_rgba(52,211,153,0.5)]" />
    </div>
    <div className="flex flex-col">
    <h2 className="text-body font-ui flex items-center gap-1.5 text-paper tracking-wide">
   ...
   <form onSubmit={handleChat} className="p-4 border-t border-bone/5 bg-char/10 flex items-center z-10">
    ...
    <Input 
     ...
     className="w-full bg-void/50 border border-bone/10 focus-visible:border-indigo-haze/50 text-[14px] text-white placeholder:text-mist/50 focus-visible:ring-0 pl-4 pr-12 h-11 rounded-lg transition-colors -inner"
    ...
    <Button 
     ...
     className="absolute right-1.5 top-1.5 h-8 w-8 rounded-md bg-indigo-haze text-paper hover:bg-indigo-haze/80 transition-colors disabled:opacity-30 disabled:hover:scale-100 disabled:bg-indigo-haze/30"
    >
