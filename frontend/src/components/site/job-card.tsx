...
export function JobCard({ job, matchScore }: { job: JobWithCompany; matchScore?: number }) {
 ...
 <motion.div whileHover={{ y: -2 }} transition={{ duration: 0.2 }} className="group">
  <div className="glass-card flex flex-col p-5 transition-colors hover:bg-iron/40 hover:border-bone/20">
  <div className="flex gap-4">
   <Avatar className="h-12 w-12 rounded-md bg-void border border-bone/10 shrink-0">
   <AvatarImage src={job.company.logoUrl || ""} />
   <AvatarFallback className="rounded-md font-display text-void bg-bone">{job.company.name.charAt(0)}</AvatarFallback>
   </Avatar>
   
   <div className="flex-1 space-y-3">
   <div>
    <div className="flex items-center justify-between">
    <h3 className="font-geist text-[18px] font-ui text-paper group-hover:text-paper/90 transition-colors">
     {job.title}
    </h3>
    {isMatch && (
   ...
   {(job.tags?.length > 0 || job.requirements?.length > 0) && (
    ...
    {(job.tags || job.requirements).slice(0, 4).map((tag) => (
     <Badge key={tag} variant="outline" className="rounded-pill border-bone/10 bg-iron/40 text-mist px-2.5 font-body">
     {tag}
     </Badge>
    ))}
    </div>
