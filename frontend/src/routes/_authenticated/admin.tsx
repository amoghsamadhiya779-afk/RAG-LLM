  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      <main className="flex-1 container-page py-10">
        <h1 className="font-display text-h2 font-display">Admin</h1>
        ...
        <h2 className="mt-10 font-display text-body-lg font-heading">Pending moderation</h2>
        {isLoading ? (
          <div className="mt-4 space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="glass-card h-24 animate-pulse" />)}</div>
        ) : pending.length === 0 ? (
          <div className="mt-4 glass-card p-10 text-center text-secondary">Nothing pending. Inbox zero ✨</div>
        ) : (
          <ul className="mt-4 space-y-3">
            {pending.map((j) => (
              <li key={j.id} className="glass-card flex items-start justify-between gap-4 p-4">
                <div className="min-w-0">
                  <h3 className="font-ui">{j.title}</h3>
                  <p className="text-micro text-secondary">{j.company.name} · {j.location ?? "Remote"}</p>
                  <p className="mt-2 line-clamp-2 text-small text-secondary">{j.description}</p>
                </div>
...
function StatTile({ icon: Icon, label, value }: { icon: typeof Briefcase; label: string; value: number }) {
  return (
    <div className="glass-card p-4">
      <div className="flex items-center gap-2 text-micro text-secondary">
        <Icon className="h-3.5 w-3.5" /> {label}
      </div>
      <div className="mt-1 font-display text-h3 font-heading">{value}</div>
    </div>
  );
}
