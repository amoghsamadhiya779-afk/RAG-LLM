      ...
      <main>
        <section className="container-page pt-24 pb-20">
          <motion.div initial="hidden" whileInView="show" viewport={viewportOnce} variants={revealStagger} className="max-w-3xl">
            ...
            <motion.p variants={reveal} className="mt-6 text-lg leading-relaxed text-muted-foreground">
        ...
        <section className="container-page py-20">
          <motion.div initial="hidden" whileInView="show" viewport={viewportOnce} variants={revealStagger} className="grid gap-6 md:grid-cols-3">
            {values.map((v) => (
              <motion.div key={v.title} variants={reveal} className="rounded-2xl border border-border bg-card p-8">
        ...
        <section className="container-page py-20">
          <div className="mb-10">
            ...
            <h2 className="h-display mt-3 text-3xl md:text-4xl">Small team. Big roadmap.</h2>
          </div>
          <motion.div initial="hidden" whileInView="show" viewport={viewportOnce} variants={revealStagger} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {team.map((m) => (
              <motion.div key={m.name} variants={reveal} className="rounded-2xl border border-border bg-card p-6">
                <div className="grid h-14 w-14 place-items-center rounded-full bg-gradient-to-br from-indigo-500 via-violet-500 to-pink-500 text-sm font-semibold text-white">
                  {m.initials}
                </div>
                <div className="mt-5 text-base font-semibold">{m.name}</div>
