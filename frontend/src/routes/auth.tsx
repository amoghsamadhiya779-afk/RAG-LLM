      ...
      <main className="flex-1 grid place-items-center px-4 py-12">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="glass-card p-8">
            <div className="mb-6 flex rounded-md bg-secondary p-1 text-sm">
              <button onClick={() => setMode("in")} className={`flex-1 rounded px-3 py-1.5 ${mode === "in" ? "bg-background shadow-sm" : "text-muted-foreground"}`}>Sign in</button>
              <button onClick={() => setMode("up")} className={`flex-1 rounded px-3 py-1.5 ${mode === "up" ? "bg-background shadow-sm" : "text-muted-foreground"}`}>Create account</button>
            </div>

            <h1 className="font-display text-2xl font-semibold">{mode === "in" ? "Welcome back" : "Join jOBiON"}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{mode === "in" ? "Sign in to apply and track replies." : "Free for candidates. Always."}</p>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="mt-1 grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => setRole("seeker")} className={`rounded-md border px-3 py-2 text-sm ${role === "seeker" ? "border-primary bg-primary/10 text-primary" : "border-border"}`}>Job seeker</button>
                      <button type="button" onClick={() => setRole("employer")} className={`rounded-md border px-3 py-2 text-sm ${role === "employer" ? "border-primary bg-primary/10 text-primary" : "border-border"}`}>Employer</button>
                    </div>
                  </div>
                </>
              )}
