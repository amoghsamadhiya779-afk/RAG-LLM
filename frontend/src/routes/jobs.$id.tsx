...
export const Route = createFileRoute("/jobs/$id")({
  loader: async ({ params }) => {
    const job = await api.jobs.get(params.id).catch(() => null);
    if (!job) throw notFound();
    return { job };
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: loaderData ? `${loaderData.job.title} at ${loaderData.job.company.name} — jOBiON` : "Job — jOBiON" },
    ...
    scripts: loaderData
      ? [{
          ...
          children: JSON.stringify({
            ...
            description: loaderData.job.description,
            datePosted: loaderData.job.createdAt,
            employmentType: loaderData.job.jobType.toUpperCase(),
            hiringOrganization: { "@type": "Organization", name: loaderData.job.company.name },
            jobLocation: { "@type": "Place", address: { "@type": "PostalAddress", addressLocality: loaderData.job.location ?? "Remote" } },
          }),
        }]
      : [],
  }),
...
function JobDetail() {
  ...
  const { data: similar } = useQuery({
    queryKey: ["jobs", "similar", job.id],
    queryFn: () => api.jobs.similar(job.id),
  });
