 ...
 return (
  <div className="min-h-dvh flex flex-col">
   ...
   <main className="flex-1 container-page py-10">
    <div className="mb-6 flex items-center gap-3 rounded-pill border border-bone/10 bg-char px-4 py-2 ">
    ...
    <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
     <aside className="space-y-6">
      <div className="flex items-center gap-2 text-small font-ui">
       <SlidersHorizontal className="h-4 w-4" /> Filters
      </div>

      <div>
       <div className="mb-2 text-micro font-ui uppercase text-secondary">Stack</div>
       <div className="flex flex-wrap gap-1.5">
        {ALL_TAGS.map((t) => (
         <button
          key={t}
          onClick={() => toggleTag(t)}
          className={`rounded-pill px-3 py-1 text-micro transition-colors ${tags.includes(t) ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}
         >
          {t}
         </button>
        ))}
      ...
      <div>
       <div className="mb-2 text-micro font-ui uppercase text-secondary">Type</div>
       <div className="flex flex-wrap gap-1.5">
        {TYPES.map((t) => (
         <button
          key={t.v}
          onClick={() => setJobType(jobType === t.v ? undefined : t.v)}
          className={`rounded-pill px-3 py-1 text-micro ${jobType === t.v ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
         >{t.label}</button>
        ))}
       </div>
      </div>
      ...
      <div>
       <div className="mb-2 text-micro font-ui uppercase text-secondary">Level</div>
       <div className="flex flex-wrap gap-1.5">
        {LEVELS.map((l) => (
         <button
          key={l}
          onClick={() => setLevel(level === l ? undefined : l)}
          className={`rounded-pill px-3 py-1 text-micro capitalize ${level === l ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
         >{l}</button>
        ))}
       </div>
      </div>
      ...
      <div>
       <div className="mb-2 flex items-center justify-between text-micro uppercase text-secondary">
     ...
     <section>
      <div className="mb-4 text-small text-secondary">
