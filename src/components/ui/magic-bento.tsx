...
const demoCards: BentoCardData[] = [
  { color: "#0a0f14", title: "Analytics", description: "Track user behavior", label: "Insights" },
  { color: "#0a0f14", title: "Dashboard", description: "Centralized data view", label: "Overview" },
  { color: "#0a0f14", title: "Collaboration", description: "Work together seamlessly", label: "Teamwork" },
  { color: "#0a0f14", title: "Automation", description: "Streamline workflows", label: "Efficiency" },
  { color: "#0a0f14", title: "Integration", description: "Connect favorite tools", label: "Connectivity" },
  { color: "#0a0f14", title: "Security", description: "Enterprise-grade protection", label: "Protection" },
];
...
const ParticleCard = ({
  ...
  const commonProps = {
    ref: cardRef as RefObject<HTMLAnchorElement & HTMLDivElement>,
  ...
  if (href) {
    return (
      <Link to={href as never} {...commonProps}>
        {children}
      </Link>
    );
  }
      ...
      <div ref={gridRef} className="card-grid bento-section">
        {data.map((card, index) => {
          ...
          const style: CSSProperties = {
            backgroundColor: card.color ?? "#0a0f14",
          };
          ...
          if (card.href) {
            return (
              <Link key={index} to={card.href as never} className={baseClassName} style={style}>
                {content}
              </Link>
            );
          }
